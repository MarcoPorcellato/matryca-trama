...
| F | Trama | E plus exact released Plumber profile | client adapter, TCK parity, no Parser/host imports merge |
...
fourth source repository. Its proposed root is `contracts/python/`; Task 4
must reject that location if an isolated build imports `src`, `frontend`,
Parser, or a Logseq adapter.
...
- [ ] **Step 4: Add the decision and correct superseded language**

Write the ADR with status `Accepted`, exact source authority rules, full
...
```bash
rtk git add docs/decisions docs/contracts docs/superpowers/plans docs/roadmaps docs/knowledge tests/test_docs_gateway_authority.py
...
## Task 4: Plumber publishes canonical `plumber.graph.read/v1` material and TCK
...
def test_binding_has_no_runtime_or_source_adapter_imports() -> None:
    source = Path("contracts/python/src/matryca_plumber_contracts").read_text()
...
```bash
rtk uv run pytest contracts/python/tests/test_binding.py contracts/tck/tests/test_runner.py -q
rtk uv run python contracts/tck/runner.py --fixtures contracts/plumber.graph.read/v1/fixtures
```

Expected: FAIL because schema, manifest, vectors, binding, and runner are absent.
...
success/failure. It rejects parser fields, absolute paths, raw queries, and
unbounded payloads. The manifest hashes every fixture and schema using
lowercase SHA-256. The runner validates positive and negative vectors offline;
it never opens a vault, host, network connection, or cache.
...
```bash
rtk uv build contracts/python
rtk uv run --isolated --no-project python -c "import matryca_plumber_contracts; print(matryca_plumber_contracts.CONTRACT_ID)"
```
...
```bash
rtk git add docs/contracts contracts CHANGELOG.md
...
- Produces: `GraphSessionReadPort` (`open_session`, `read`, `close_session`),
  `OgParserSourceAdapter`, and additive local transport operation.
...
and receipt metadata. `OgParserSourceAdapter` receives only a bounded selected
...
Register a new `plumber_graph_read_v1` tool in `register_mcp_tools`. It accepts
only a serialized canonical request and returns only serialized canonical
result. It delegates once to `PlumberGraphReadService`; it does not alter
...
Expected: accepted vectors pass; stale, foreign, unsupported, malformed,
missing-provenance, incomplete-subtree, and excessive-limit vectors fail
explicitly; pre-existing OG/Shadow tests remain green.

- [ ] **Step 6: Commit OG vertical**

```bash
rtk git add src/graph/ports/session_read.py src/agent/plumber_graph_read_service.py src/agent/og_parser_source_adapter.py src/agent/mcp_server.py tests docs/contracts CHANGELOG.md
rtk git commit -m "feat(graph): serve OG reads through Plumber contracts"
```

## Task 6: Publish exact Plumber profile, then make Trama a consumer

**Repositories:** Plumber publication gate first; then Trama from its newly
fetched `origin/main`. Do not combine these repositories in one branch or PR.

**Plumber publication evidence files:**
- Create: `docs/contracts/evidence/plumber-graph-read-v1-<version>.md`
- Create: `docs/contracts/evidence/plumber-graph-read-v1-<version>.sbom.json`
- Create: `docs/contracts/evidence/plumber-graph-read-v1-<version>.sha256`
...
- Delete after replacement passes: `packages/parser-bridge/`,
  `packages/logseq-og-adapter/`, and their direct-producer tests.

**Interfaces:**
- Consumes: one released `matryca-plumber-contracts` version; exact artifact,
  schema, fixture, SBOM, notice, and release-provenance digests.
- Produces: Trama `KnowledgeGraphGateway` values and one outer
  `PlumberClientAdapter`; no direct Parser or storage/host access.

- [ ] **Step 1: Stop unless publication evidence is complete**

Require a signed/reviewed evidence row containing all values below before
starting Trama code:

```text
plumber_commit=<complete SHA>
contract_version=<exact SemVer>
artifact_sha256=<64 lowercase hex>
schema_sha256=<64 lowercase hex>
fixture_sha256=<64 lowercase hex>
sbom_sha256=<64 lowercase hex>
notice_sha256=<64 lowercase hex>
release_provenance=<immutable URL or digest>
```
...
- [ ] **Step 2: Write Trama's failing consumer tests**
...
- [ ] **Step 4: Add the outer adapter and remove replaced Trama source path**
...
`trama-logseq-og-adapter`, and `trama.logseq.read/v1` producer source in the
same commit. Retain their historical documentation with an explicit archival
status and source anchor; do not claim those tests qualified the new profile.

- [ ] **Step 5: Run exact vector, architecture, and full Trama gates**
...
- [ ] **Step 6: Commit profile-specific Trama migration**

```bash
rtk git add architecture.toml pyproject.toml packages docs tests
rtk git rm -r packages/parser-bridge packages/logseq-og-adapter
...
- Modify: `tests/test_cli.py` or current command-specific CLI test module
- Modify: `README.md`
- Modify: `docs/ARCHITECTURE.md`
- Modify: `docs/COOKBOOK.md`
- Modify: `docs/reference/API_STABILITY.md`
...
```bash
rtk uv run pytest tests/test_lens.py tests/test_cli.py -q
```
...
```bash
rtk uv sync --locked --all-extras
rtk uv run pytest tests/test_lens.py tests/test_cli.py tests/test_layer_boundary.py -q
rtk make all
rtk make vendor-name-check
rtk git diff --check
```
...
**Interfaces:**
- Consumes: released `plumber.graph.read/v1` binding/session/graph identifiers.
...
def test_navigation_requires_user_intent_and_supported_host() -> None:
    ...
    assert run_vector("cancelled-request") == "timeout"
```
...
```bash
rtk uv run pytest contracts/tck/tests/test_topology_navigation_runner.py -q
```
...
```bash
rtk uv run pytest contracts/tck/tests/test_topology_navigation_runner.py -q
rtk uv run python contracts/tck/runner.py --fixtures contracts/plumber.graph.topology/v1/fixtures
rtk uv run python contracts/tck/runner.py --fixtures contracts/plumber.host.navigate/v1/fixtures
rtk make check
rtk git diff --check
```
...
**Files:**
- Create when evidence exists: `docs/contracts/evidence/logseq-db-host-<profile>.md`
...
```text
official_host_name=<official product surface>
artifact_version=<immutable version>
artifact_sha256=<64 lowercase hex>
transport=<CLI | official SDK | approved local transport>
synthetic_graph_digest=<64 lowercase hex>
...
6. Task 6 Plumber profile publication evidence, then separate Trama consumer PR.
...
| Task 6 | one Plumber evidence commit; one Trama consumer commit | multi-repo commit, Brain code |
...
- After Task 6: disable the exact Trama Plumber profile and show a truthful
  unavailable state. Do not resurrect Trama Parser/OG source as a hidden
  fallback.
...
| existing OG path through Parser public API, sessions and failure rules | 5 |
